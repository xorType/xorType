## Hi 👋, I'm Matthias Blackshear

I work between engineering, product, and leadership, ensuring technical initiatives actually ship and align with business goals. In the GenAI era, I use AI to write in-house solutions for project, program, product, PMO, and leadership roles using Go, MongoDB, Embedding Models (Qwen, Nomic, Anthropic, OpenAI, Voyage AI), LLMs (Claude, ChatGPT, Qwen, Copilot), Agents & Skills, and small language models on Ollama (basically anything my GPU/CPU can handle and cloud models). Claude is my go-to ecosystem but I'm just as handy everywhere else. 

#### What I do

* I deliver technical solutions, utilizing my full experience as a full-stack developer, architect, and entrepreneur.
* I use my background to sit as a strategic PM that finds balance in **strategy** and tactical execution.
* I utilize AI and agentic processes to the fullest extent to 10x my work productivity.
* As a TPM in the AI era, I write software solutions to augment my daily workflow.
* Project Management: I see shit (risks) before it happens, lean in, and work collaboratively to solve problems.
* Program Management: (depends)
  * **Product Company:** I own the entire program that **supports and enables** the delivery of products used by millions.  
  * **Enterprise:** I manage massive programs comprised of multiple projects where combined budgets **exceed** $100M, **mixed** with plenty of internal politics.
* Product Management: I have worked many years to ideate, **research**, and market-test products and incubate long-term **roadmaps**.
* GenAI Solutions: Given my background in ML and AI, solution architecture, cloud, software development, and creativity, the GenAI era has **opened** the door for me to be more open in my intentions and ability.

## Industries I've Worked In
Realistically, there is no industry that I cannot wrap my head around though. 

* Database & Infrastructure
* Auto Retail Technology & SaaS
* Big 4 Consulting Services
* Consumer Services & Utility Marketing
* Public Health & Government
* Manufacturing & Consumer Goods
* Consulting & Professional Services
* Insurance Claims
* Logistics & Supply Chain
* Construction Data & Analytics
<!--
**xorType/xorType** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

